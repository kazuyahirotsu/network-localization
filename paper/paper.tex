...
We use a simple four\textemdash step loop: (1) estimate path\textemdash loss on anchor\textendash anchor links; (2) convert RSSI to distances for links incident to unknowns; (3) multilaterate unknown nodes with bounded least\textemdash squares; (4) promote confident estimates to anchors and repeat.

\begin{enumerate}
  \item \textbf{Estimate path\textemdash loss from anchors.} Fit the log\textemdash distance model to anchor\textendash anchor RSSI averaged over $K$ samples; use bounds and fallbacks:
  \begin{equation}\label{eq:mlat_rssi_model}
  \mathrm{RSSI}(d) = A - 10\,n\,\log_{10}(d).
  \end{equation}
  \item \textbf{Convert RSSI to distances.} For each link that touches an unknown node, convert the averaged RSSI to distance using
  \begin{equation}\label{eq:mlat_rssi2dist}
  \hat d = 10^{\frac{A - \overline{\mathrm{RSSI}}}{10\,n}}.
  \end{equation}
  Drop non\textemdash finite or implausibly large values.
  \item \textbf{Solve positions for unknowns.} For each unknown with three or more neighbors, solve a bounded 2D least\textemdash squares problem that matches these distances:
  \begin{equation}\label{eq:mlat_ls}
  \hat{\mathbf{p}}_i \,=\, \arg\min_{\mathbf{p}\in\mathcal{B}} \sum_{j} \Big(\lVert \mathbf{p} - \mathbf{q}_j\rVert_2 \,-\, \hat d_{ij}\Big)^2,
  \end{equation}
  where $\mathbf{q}_j$ are fixed neighbor coordinates (anchors or current estimates) and $\mathcal{B}$ are wide map\textemdash based bounds. Initialize at the current coordinate; keep anchors fixed.
  \item \textbf{Promote and repeat.} Promote low\textemdash residual estimates to anchors and repeat Steps 2\textendash4 for $T{=}10$. Output the final coordinates.
\end{enumerate}
...
The stark performance difference between methods stems from the fundamental challenge of inverting RSSI to distance in a terrain-rich environment. As shown in Fig.~\ref{fig:rssi_variance}, the relationship between RSSI and distance is not a simple function but a noisy, multi-modal distribution. This ill-posed inversion is the primary source of failure for iterative multilateration, which depends on accurate initial distance estimates. By committing to a single, often incorrect, distance for each RSSI measurement, it attempts to solve a geometrically inconsistent system, resulting in the catastrophic outliers evident in its error distribution (Fig.~\ref{fig:hist_nonfree}).

Our proposed model overcomes this by reframing the problem. Instead of performing a hard RSSI\textrightarrow distance conversion, it uses the trainable path\textemdash loss module to generate a distance estimate that serves as a soft, physically\textemdash grounded prior. Critically, this estimate is supplied to the GCN as just one feature alongside the raw RSSI time series. The edge\textemdash conditioned convolution can then learn to weigh the physical prior against the underlying signal properties, effectively learning to trust or distrust the distance estimate on a per\textemdash edge basis. This provides the graph network with an essential understanding of physical scale that the plain GCN, which sees only unscaled RSSI values, fundamentally lacks.

Furthermore, the GCN architecture itself provides powerful regularization by enforcing local consistency through message passing. Unlike multilateration, which computes each node's position independently, the GCN performs collective inference across the entire graph. A node’s predicted position is constrained by its neighbors, smoothing the predictions and preventing the kind of large, isolated errors that plague the baseline (Fig.~\ref{fig:qualitative_mlat_nonfree}). This graph\textemdash based reasoning is responsible for the dramatic reduction in the error tail (Table~\ref{tab:main_results}, P90/P95), yielding a model that is not only more accurate on average but also substantially more robust.

The free\textemdash space results complete this picture. In an idealized environment where the log\textemdash distance model is correct (Fig.~\ref{fig:rssi_variance}b), multilateration's direct optimization of a well\textemdash specified geometric cost function is superior. Our GCN, while still effective, is a more general function approximator and its global path\textemdash loss parameters cannot perfectly capture the geometry of every single graph instance as well as a direct least\textemdash squares fit can. This confirms that the proposed model's strength lies in its ability to robustly handle the uncertainty inherent in realistic, terrain\textemdash aware propagation environments.
