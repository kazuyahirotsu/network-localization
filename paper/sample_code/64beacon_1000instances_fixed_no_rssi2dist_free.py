...
# We will use the raw RSSI measurements as edge features.
# data.edge_attr already has shape (num_edges, num_measurements).
edge_in_dim = num_measurements
hidden_dim = 64
output_dim = 2
...
for epoch in range(num_epochs):
    ...
    for data in train_loader:
        ...
        out = main_gnn(data.x, data.edge_index, data.edge_attr)
...
with torch.no_grad():
    ...
    out = main_gnn(sample_data.x, sample_data.edge_index, sample_data.edge_attr)
    predicted_scaled = out.cpu().numpy()
    predicted_positions = y_scaler.inverse_transform(predicted_scaled)
...
#######################################
save_path = 'trained_localization_model_64beacons_1000instances_fixed_no_rssi_free.pth'
torch.save({
