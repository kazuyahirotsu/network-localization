...
This thesis investigates the feasibility of using LoRa networks as a standalone positioning infrastructure for GNSS-denied environments.
The objective is to propose and evaluate an integrated system that addresses both challenges: robust UAV localization given beacon positions, and infrastructure self-localization when beacon positions are unknown.

To address these challenges, we propose a two-layer architecture.
For \textbf{client-side localization}, we improve standard multilateration with two modifications: (1) altitude constraints that fix the vertical coordinate using barometer or laser altimeter data, reducing the problem from 3D to 2D and preventing vertical errors from corrupting the horizontal solution, and (2) the Huber loss function that limits the influence of RSSI outliers by applying linear rather than quadratic penalties to large errors.
For \textbf{infrastructure self-localization}, we propose a physics-aware Graph Convolutional Network (GCN) that learns to estimate beacon positions from peer-to-peer RSSI measurements.
Unlike black-box neural networks, our architecture embeds a trainable log-distance path-loss model, learning propagation parameters jointly with the graph structure, and employs edge-conditioned convolutions that dynamically weight each link based on its reliability.
...
\begin{enumerate}
    ...
    \item We present a feasibility analysis of the integrated system, showing that despite mean UAV localization errors of approximately 120 to 160 meters when using beacon positions with 200 meters mean error, point-to-point navigation, circular patrol, and area coverage missions remain achievable.
\end{enumerate}
...
\begin{enumerate}
    \item \textbf{Altitude Decoupling:} Modern UAVs carry reliable barometers or laser altimeters. We fix the UAV altitude using this sensor value and estimate only the horizontal coordinates $(X, Y)$. This reduces the position estimation problem from three dimensions to two and prevents vertical errors from affecting the horizontal estimate.
...
\begin{figure}[t]
    \centering
    \includegraphics[width=0.85\columnwidth]{method_comparison.pdf}
...
\begin{figure}[t]
    ...
    \begin{tabular}{@{}c@{\hspace{1mm}}c@{\hspace{1mm}}c@{}}
        \includegraphics[width=0.31\columnwidth]{images/sample_visualization_loaded_model_trained_localization_model_64beacons_1000instances_fixed_power13.pdf} &
        \includegraphics[width=0.31\columnwidth]{images/sample_visualization_loaded_model_trained_localization_model_64beacons_1000instances_fixed_no_rssi2dist.pdf} &
        \includegraphics[width=0.31\columnwidth]{images/sample_visualization_loaded_model_mlat_64beacons_100instances.pdf} \\[-1mm]
...
\begin{figure}[t]
    \centering
    \begin{tabular}{cc}
        \includegraphics[width=0.48\columnwidth]{images/diagonal_beacon0m_path.pdf} &
        \includegraphics[width=0.48\columnwidth]{images/diagonal_beacon200m_path.pdf} \\
