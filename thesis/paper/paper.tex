...
\begin{eabstract}
    ...
    Adding beacon position error of 200~m mean 2D error (matching the GCN's self-localization performance of 206~m) increases the UAV's localization error from a baseline of 122~m to 157~m, a degradation of only 29\%.
    This sub-linear degradation confirms system stability: errors in the infrastructure do not cause catastrophic failure on the client side.
    ...
    \begin{enumerate}
        \item \textbf{Geometric Stability:} When the UAV is more than 1~km from its target, a 200~m position error translates to a worst-case heading error of only \textbf{11 degrees}.
        ...
        \item \textbf{Mission Success:} All three mission profiles achieve 100\% waypoint success at beacon error levels up to 200~m.
    ...
    infrastructure.
    The resulting system supports disaster-response and remote sensing operations, allowing autonomous missions to continue when primary navigation fails.
\end{eabstract}
...
For the past several decades, Global Navigation Satellite Systems (GNSS), exemplified by the U.S. Global Positioning System (GPS), have served as the dominant positioning technology for outdoor applications~\cite{ref_gps_analysis}.
...
Among RF technologies, Low-Power Wide-Area Network (LPWAN) protocols have attracted attention for their combination of long communication range, low power consumption, and minimal hardware cost~\cite{ref_lpwan_survey}.
LoRa (Long Range), a leading LPWAN technology, achieves communication distances of 2--15~km in line-of-sight conditions while consuming milliwatts of power and requiring only low-cost transceiver modules~\cite{ref_lora_survey}.
These characteristics make LoRa attractive as the foundation for a backup positioning infrastructure that can be rapidly deployed in GNSS-denied regions.
...
Time-of-Flight (ToF) and Time-Difference-of-Arrival (TDoA) methods measure signal propagation time to estimate distance, achieving meter-level accuracy~\cite{ref_tdoa_lora}.
...
Angle-of-Arrival (AoA) methods determine bearing to signal sources but require complex antenna arrays at each beacon~\cite{ref_aoa_lora}.
...
However, in GNSS-denied environments, the very scenarios motivating this research, these calibration methods become unavailable.

Consider an emergency response scenario where a beacon network must be established rapidly in a disaster zone or remote area.
...
A small subset of nodes may have known positions (anchors), perhaps established at the perimeter of the denied zone or recorded during brief windows of GNSS availability, but the majority must estimate their coordinates from peer-to-peer RSSI measurements.
Classical geometric solvers such as iterative multilateration suffer from severe error propagation in sparse networks with complex terrain, where early estimation errors compound catastrophically through subsequent iterations~\cite{ref_iterative_mlat}.
...
In this thesis, we demonstrate that a carefully designed system architecture, combined with algorithmic innovations at both the client and infrastructure layers, can provide operationally useful positioning in GNSS-denied environments despite the inherent limitations of RSSI-based localization.
...
\begin{enumerate}
    ...
    Through comprehensive simulations and field experiments, we demonstrate median localization errors of 4.5~m in static conditions and 62.9~m during dynamic flight at 10~m/s, representing a 25\% improvement over conventional least-squares multilateration.
    ...
    We present a comprehensive analysis of the complete system, from infrastructure self-organization through UAV mission execution.
    Through closed-loop navigation simulations, we demonstrate that despite localization errors on the order of 150--200~m, the system maintains operational utility through geometric stability: at 1~km from target, even 200~m position error produces at most 11 degrees of heading error.
    We identify the ``turning point problem'' where accuracy degrades near waypoints and propose mitigation strategies including relaxed arrival thresholds and overlapping flight paths for area coverage missions.
\end{enumerate}
...
We instead propose an integrated architecture that builds positioning capability from scratch using only commodity LoRa hardware and algorithmic innovations.
...
We formulate the beacon self-localization task as a graph learning problem, present the physics-aware GCN architecture, and evaluate performance against baselines using terrain-aware Longley-Rice simulations.
...
We analyze sensitivity to infrastructure errors, demonstrate geometric stability through closed-loop simulations, and propose deployment strategies for practical applications.
...
RSSI measures the power level of a received signal in dBm, which attenuates with distance according to the log-distance path-loss model~\cite{vazquez2020rssi}:
\begin{equation}
...
Time-Difference-of-Arrival (TDoA) uses the difference in arrival times at multiple receivers to hyperbolic position fixing, eliminating the need for transmitter-receiver synchronization~\cite{pospisil2020tdoa}.
...
Fargas and Petersen~\cite{fargas2017gps} demonstrate GPS-free geolocation using LoRa TDoA with promising results.
...
Baik et al.~\cite{ref_aoa_lora} demonstrate a hybrid RSSI-AoA system achieving approximately 3~m accuracy in a 55~m $\times$ 101~m field.
...
Fan and Yan~\cite{fan2023csi} demonstrate CSI-based indoor localization using graph structure fusion.
...
Given the constraints of GNSS-denied environments and the goal of rapid, low-cost deployment, this thesis focuses on RSSI-based localization using LoRa technology.
...
Yucer et al. report RSSI-based outdoor localization with a single UAV, demonstrating the concept but with limited accuracy.
...
Our approach differs by achieving robustness through algorithmic improvements alone:
...
These characteristics limit fingerprinting's applicability to scenarios requiring rapid deployment in unknown environments, such as disaster response or remote monitoring.

\paragraph{Machine Learning for RSSI-Distance Mapping}
Various machine learning approaches have been applied to improve RSSI-to-distance conversion, including neural networks and random forests.
These methods can capture non-linear relationships that simple path-loss models miss.
However, they typically require labeled training data (RSSI measurements with known distances), which may be unavailable in new deployment environments.

\subsection{Graph Neural Networks for Localization}

Graph Neural Networks (GNNs) have emerged as a powerful framework for localization by explicitly modeling the wireless network as a graph, where nodes represent devices and edges represent communication links~\cite{yan2021gnn}.
...
Simonovsky and Komodakis~\cite{simonovsky2017nnconv} introduce dynamic edge-conditioned filters for graph convolutions, where edge attributes shape the message-passing weights.
This architecture enables the network to learn distance-aware aggregation functions when edge features encode proximity information.
...
\begin{itemize}
    ...
    \item \textbf{Outdoor LPWA Focus:} We specifically target sparse outdoor networks using LoRa, where node density is low (4 nodes/km$^2$) and terrain effects dominate signal propagation.
...
Our contributions at both the client layer (Chapter~\ref{chap:uav_localization}) and infrastructure layer (Chapter~\ref{chap:infrastructure}) target this gap through algorithmic innovations rather than hardware enhancements.
...
\paragraph{Rapid Deployment}
The system must be deployable within hours rather than days, enabling effective response to emergent situations such as natural disasters or remote expeditions.
This requirement precludes approaches that depend on extensive site surveys, pre-existing infrastructure, or complex calibration procedures.
...
The system should utilize commodity hardware, specifically single-antenna LoRa transceivers, to minimize deployment costs and enable rapid procurement.
...
\begin{figure}[htbp]
    \centering
    % TODO: Create figure showing asymmetric communication model
    % - Infrastructure (Beacons) box on left
    % - Client (UAVs) box on right  
    % - Hub (Base Station) box below
    % - Solid arrow: Beacons -> UAVs (beacon signals)
    % - Dashed arrow: UAVs -> Beacons (wake-up, optional)
    % - Bidirectional arrow: Hub <-> Beacons (RSSI data / Positions)
    \fbox{\parbox{0.8\textwidth}{\centering\vspace{3cm}[Figure: Communication Model]\vspace{3cm}}}
    \caption[Asymmetric communication model]{Asymmetric communication model. Solid arrows indicate primary data flow; dashed arrows indicate optional messages.}
    \label{fig:comm_model}
\end{figure}

As illustrated in Figure~\ref{fig:comm_model}, beacons broadcast their positions, and UAVs passively receive these broadcasts without transmitting.
...
These beacons serve as position references for client UAVs, analogous to the role of satellites in GNSS but at much shorter ranges.
...
\begin{itemize}
    ...
    \item A power supply (battery, solar panel, or mains connection)
...
During initial deployment or periodic recalibration, beacons measure RSSI values from neighboring beacons and report these measurements to the central hub.
...
Using this graph, the hub executes the infrastructure self-localization algorithm (Chapter~\ref{chap:infrastructure}) to estimate beacon positions.
...
The self-organization process typically completes within minutes, after which estimated positions are distributed back to beacons.
...
\begin{itemize}
    ...
    \item Optional: uncertainty estimate or quality indicator
\end{itemize}
...
We incorporate a wake-on-demand mechanism:

\begin{enumerate}
    \item \textbf{Sleep State:} By default, beacons enter deep sleep mode with only a low-power wake-up receiver active, consuming microamps of current.
    
    \item \textbf{Wake-up Signal:} When a UAV enters an area and requires positioning service, it broadcasts a single wake-up signal. This is the only transmission required from UAVs.
...
This mechanism enables month-long battery operation while providing on-demand service.
Multiple UAVs benefit from a single wake-up signal, and subsequent UAVs entering the area can extend the active period with additional wake-up transmissions.
...
The LoRa receiver can be the same module used for telemetry or command links in many UAV platforms, requiring minimal additional hardware.
...
\begin{enumerate}
    ...
    \item \textbf{Position Estimation:} When sufficient measurements are available (minimum 3 beacons for 2D, 4 for 3D without altitude constraint), the UAV executes the multilateration algorithm described in Chapter~\ref{chap:uav_localization}.
...
The passive reception model provides theoretical unlimited scalability.
Consider a scenario with $B$ beacons each transmitting at average interval $T$ seconds with packet duration $\tau$:

\begin{equation}
    \text{Channel utilization} = \frac{B \cdot \tau}{T}
\end{equation}

For $B = 20$ beacons, $\tau = 100$~ms packets, and $T = 3$~s average interval, channel utilization is approximately 0.67\%, leaving ample capacity for the channel regardless of how many UAVs are receiving.

In contrast, a symmetric model where each of $N$ UAVs transmits would have utilization proportional to $N$, creating congestion as the UAV count increases.
...
\paragraph{Mobile Command Post}
For deployable scenarios, the hub is integrated into a vehicle-mounted command post that can relocate as operations evolve.
...
This computation is performed once during initial deployment and can be repeated periodically to account for environmental changes or beacon drift.
...
The hub manages anchor designations, ensuring the self-localization algorithm is constrained by available ground truth.
...
\begin{figure}[htbp]
    \centering
    % TODO: Create figure showing system architecture
    % - Multiple beacon nodes (circles) scattered in area
    % - Two UAV nodes (diamonds) in the area
    % - Hub node on the side
    % - Dashed lines between beacons (self-organization RSSI links)
    % - Blue arrows from beacons to UAVs (broadcast signals)
    % - Orange arrows between hub and some beacons
    % - Legend: Beacon, UAV, Hub
    \fbox{\parbox{0.8\textwidth}{\centering\vspace{4cm}[Figure: System Architecture]\vspace{4cm}}}
    \caption[System architecture overview]{System architecture showing beacons (circles), UAVs (diamonds), and hub. Dashed lines indicate beacon-to-beacon measurements during self-organization; solid blue arrows show beacon broadcasts received by UAVs; orange arrows show hub-to-beacon communication.}
...
\begin{enumerate}
    \item \textbf{Pre-Deployment Planning} (days to weeks before): Terrain data is acquired, feasibility assessment is conducted through simulation, and the GCN model is trained. The trained model is loaded onto the hub.
    
    \item \textbf{Physical Deployment} (0--2 hours): Beacons are placed at suitable locations throughout the operational area. Placement can be approximate; exact surveying is not required.
    
    \item \textbf{Self-Organization} (15--30 minutes): Beacons enter Mode A, exchanging signals and reporting measurements to the hub. The hub runs inference using the pre-trained GCN to estimate beacon positions from the network graph.
    
    \item \textbf{Position Distribution} (5 minutes): The hub transmits estimated positions to each beacon, which store them for subsequent broadcasts.
    
    \item \textbf{Service Operation} (indefinite): Beacons enter Mode B, broadcasting positions for UAV consumption. The system is now operational for client positioning.
    
    \item \textbf{Periodic Recalibration} (optional): Self-organization can be repeated periodically (e.g., daily) to account for environmental changes affecting propagation characteristics.
\end{enumerate}
...
%===================================================================
% CHAPTER 4: ROBUST UAV LOCALIZATION
%===================================================================
\chapter{Robust UAV Localization with Altitude Constraints}
\label{chap:uav_localization}

This chapter addresses the client-side localization problem: determining the position of a mobile UAV using RSSI measurements from beacons at known locations.
...
\paragraph{Antenna Effects}
Practical antennas have non-uniform radiation patterns.
Monopole antennas commonly used in LoRa modules exhibit reduced gain directly above the antenna (the ``cone of silence''), causing signal strength variations that depend on geometry rather than distance alone.
...
\begin{enumerate}
    ...
    \item Provides a mechanism to detect and reject unreliable estimates
\end{enumerate}
...
The proposed system places multiple beacons at elevated positions (rooftops, towers, or poles) throughout the operational area.
...
Received measurements are cached in a sliding window, and localization is performed when sufficient measurements from distinct beacons are available (minimum 3 for 2D with altitude constraint, 4 for unconstrained 3D).
...
More sophisticated models incorporating altitude or terrain effects are discussed in Section~\ref{sec:uav_discussion}.
...
A key insight is that UAVs can obtain reliable altitude information from onboard sensors independent of the RF positioning system:
\begin{itemize}
...
\begin{enumerate}
    \item Initialize $(\hat{X}, \hat{Y})$ at the centroid of beacon positions
...
This factorial design allows us to isolate the individual contributions of each improvement (Huber loss and altitude constraints) as well as their combined effect.
...
These results are obtained by generating all possible combinations of beacon subsets and computing localization error for each combination.

\begin{figure}[htbp]
    \centering
    \begin{tabular}{c}
        \includegraphics[width=0.8\columnwidth]{images/method1_2d.png} \\
        (a) 2D error \\[2mm]
        \includegraphics[width=0.8\columnwidth]{images/method1_3d.png} \\
        (b) 3D error
    \end{tabular}
...
\begin{figure}[htbp]
    \centering
    \begin{tabular}{c}
        \includegraphics[width=0.8\columnwidth]{images/method3_2d.png} \\
        (a) 2D error \\[2mm]
        \includegraphics[width=0.8\columnwidth]{images/method3_3d.png} \\
        (b) 3D error
    \end{tabular}
...
\begin{figure}[htbp]
    \centering
    \begin{tabular}{c}
        \includegraphics[width=0.8\columnwidth]{images/method4.png} \\
        (a) Full scale \\[2mm]
        \includegraphics[width=0.8\columnwidth]{images/method4_cropped.png} \\
        (b) Cropped view
    \end{tabular}
    \caption[Static error: Method 4 (proposed: Huber loss with altitude constraint)]{Static localization error for Method 4 (proposed: Huber loss with altitude constraint). (a) shows occasional large errors that can be detected via minimization error monitoring; (b) shows the typical error distribution achieving 4.5~m median.}
...
Comparing the 3D results in Figures~\ref{fig:static_method1} and~\ref{fig:static_method3}, methods without altitude constraints exhibit significantly larger vertical errors.
...
Method 4 combines both improvements, achieving the best performance in all metrics.
...
A striking difference emerges: methods using Huber loss (3, 4) improve monotonically with additional beacons, while methods using squared loss (1, 2) degrade.
...
Experiments were conducted in a parking area in Pergine, Italy.
Four beacons were deployed in a rectangular configuration, as shown in Figure~\ref{fig:beacon_map}, covering an area of 610~m$^2$.
...
Four flight experiments were conducted with varying control modes and trajectories:
\begin{itemize}
...
\begin{table}[htbp]
\centering
\caption{Experimental results (median error / standard deviation in meters)}
\label{tab:experiment_results}
\resizebox{\columnwidth}{!}{
\begin{tabular}{@{}lcccc@{}}
\toprule
\textbf{Exp.} & \textbf{Method 1} & \textbf{Method 2} & \textbf{Method 3} & \textbf{Method 4} \\
\midrule
1 (Manual, Oval) & 7.41/9.78 & 7.69 & 7.38/12.60 & \textbf{7.04} \\
2 (Manual, Fig-8) & 8.65/10.53 & 8.70 & 8.71/10.96 & 8.59 \\
3 (Auto, Oval) & 26.93/29.73 & 25.86 & 26.80/31.04 & \textbf{25.91} \\
4 (Auto, Fig-8) & 16.01/19.27 & 14.47 & 16.01/20.79 & \textbf{14.38} \\
\bottomrule
\end{tabular}
}
\end{table}
...
\begin{figure}[htbp]
    \centering
    \begin{tabular}{c}
        \includegraphics[width=0.8\columnwidth]{model_1-2.png} \\
        (a) Low altitude (Experiments 1--2) \\[2mm]
        \includegraphics[width=0.8\columnwidth]{model_3-4.png} \\
        (b) High altitude (Experiments 3--4)
    \end{tabular}
...
At low altitude, the expected inverse relationship holds: RSSI decreases with distance.
...
When the UAV is positioned above a beacon at high altitude, it falls within this ``cone of silence,'' receiving weaker signals despite the short distance.
This effect violates the assumptions of the simple log-distance model.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.7\columnwidth]{antenna.png}
    \caption[Signal strength variation with UAV altitude]{Signal strength variation with UAV altitude over a beacon. Higher altitude brings the UAV into the antenna's null region.}
...
\begin{enumerate}
    ...
    The 3D surface captures how signal strength varies with both horizontal distance and vertical separation, more accurately representing the antenna's radiation pattern.
...
\begin{figure}[htbp]
    ...
    \caption[Altitude-parameterized RSSI-to-distance model]{Altitude-parameterized RSSI-to-distance model. The 3D surface shows how signal strength depends on both horizontal distance and altitude difference, capturing antenna directivity effects.}
    \label{fig:model_3d}
\end{figure}
...
\begin{figure}[htbp]
    \centering
    \begin{tabular}{c}
        \includegraphics[width=0.8\columnwidth]{sample-median.png} \\
        (a) Median error \\[2mm]
        \includegraphics[width=0.8\columnwidth]{sample-std.png} \\
        (b) Standard deviation
    \end{tabular}
    \caption[Effect of sampling time on localization accuracy]{Effect of sampling time on localization accuracy. Longer sampling improves static accuracy but may introduce lag in dynamic scenarios.}
    \label{fig:sampling}
\end{figure}

Figure~\ref{fig:sampling} shows how localization accuracy varies with sampling time.
...
Adaptive sampling strategies could optimize this trade-off: using long windows during hover or slow transit, and short windows during rapid maneuvers.
...
\paragraph{Coverage Area}
LoRa's 2--15~km range enables sparse beacon deployment.
Our simulations suggest that 18 beacons/km$^2$ achieves 11.6~m median static accuracy, while 7.2 beacons/km$^2$ achieves 4.5~m accuracy.
For a 10~km$^2$ operational area, this translates to 72--180 beacons, a manageable infrastructure investment.

\paragraph{Beacon Density}
Unlike squared loss methods that degrade with additional beacons, the Huber loss approach improves monotonically.
This enables incremental deployment: starting with minimal beacons and adding more as accuracy requirements increase.
...
LoRa receivers actually consume less power than GPS receivers, offering potential battery life advantages.
...
While satellites provide global coverage, beacons provide local coverage that can be rapidly deployed and reconfigured.
...
\begin{enumerate}
    ...
    \item \textbf{Scalability:} Unlike conventional methods that degrade with additional beacons, the proposed approach improves monotonically, enabling effective use of dense beacon networks.
...
%===================================================================
\chapter{Infrastructure Self-Localization via Graph Learning}
...
A subset of beacons, called anchors, have known positions obtained through prior survey, brief GNSS access before denial, or placement at pre-determined coordinates (e.g., airdrop with GPS logging).
...
This scenario arises in rapid deployment situations where comprehensive surveying is impractical: disaster response, scientific expeditions, or remote monitoring in GNSS-challenged environments.
...
Each node $i$ has an absolute 2D position $\mathbf{p}_i = (x_i, y_i) \in \mathbb{R}^2$.
For anchors ($i \in \mathcal{A}$), this position is known; for unknown nodes ($i \in \mathcal{U}$), it must be inferred.
...
\end{equation}
where $(x_i, y_i)$ are the known coordinates for anchors and are initialized at the anchor centroid for unknown nodes.
...
This is problematic for localization, where the reliability and geometric significance of a neighbor's message depends critically on the edge characteristics (signal strength, estimated distance).
...
This allows the network to learn edge-specific transformations that account for varying link qualities and distances.
...
This architecture enables distance-aware aggregation: the network can learn to weight messages from nearby nodes (small $\hat{d}_{ij}$, strong RSSI) differently from messages from distant or obstructed nodes (large $\hat{d}_{ij}$, weak RSSI).
...
These constraints are essential because outlier RSSI values (from severe multipath or interference) or transient parameter states during early training can produce invalid distance estimates that destabilize optimization.
...
We evaluate the proposed method through comprehensive simulations using a terrain-aware propagation model that captures the challenging conditions of real outdoor deployments.
...
The limited sample budget reflects practical constraints on measurement time in rapid deployment scenarios.
...
\subsection{Propagation Regimes}

We evaluate under two propagation regimes to understand how environmental realism affects method performance:

\paragraph{Terrain-Aware Regime}
...
\paragraph{Free-Space Regime}
For comparison, we also evaluate under idealized free-space propagation with no terrain effects.
This regime reveals how environmental factors affect the relative performance of different methods.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\columnwidth]{images/rssi_variance_comparison.png}
    \caption[RSSI versus distance relationship]{RSSI versus distance relationship: (a) Terrain-aware regime showing high variance and non-linearity; (b) Free-space regime showing clean log-distance relationship.}
    \label{fig:rssi_variance}
\end{figure}

Figure~\ref{fig:rssi_variance} illustrates the stark difference between regimes.
...
\begin{enumerate}
    ...
    The objective minimizes the sum of squared differences between estimated distances and geometric distances implied by the candidate position.
    Bounds constrain solutions to lie within the deployment area.
    
    \item \textbf{Anchor Promotion:} Nodes that were successfully localized with residual error below a threshold are temporarily promoted to anchor status.
...
The plain GCN must learn the relationship between raw RSSI values and geometric distances entirely from data, without the inductive bias provided by the path-loss model.
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=\columnwidth]{images/sample_visualization_loaded_model_trained_localization_model_64beacons_1000instances_fixed_power13.png}
    \caption[Qualitative result: proposed GCN]{Qualitative localization result (terrain-aware): proposed GCN. Triangles indicate anchors; circles indicate unknown nodes. Lines connect predicted (hollow) to true (filled) positions.}
    \label{fig:qualitative_proposed}
\end{figure}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\columnwidth]{images/sample_visualization_loaded_model_trained_localization_model_64beacons_1000instances_fixed_no_rssi2dist.png}
    \caption[Qualitative result: plain GCN ablation]{Qualitative localization result (terrain-aware): plain GCN ablation (without learned distance). Errors are larger and more scattered than the proposed method.}
    \label{fig:qualitative_plain}
\end{figure}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\columnwidth]{images/sample_visualization_loaded_model_mlat_64beacons_100instances.png}
    \caption[Qualitative result: iterative multilateration]{Qualitative localization result (terrain-aware): iterative multilateration. Catastrophic errors are visible due to error propagation through anchor promotion.}
    \label{fig:qualitative_mlat}
\end{figure}

Figures~\ref{fig:qualitative_proposed}, \ref{fig:qualitative_plain}, and~\ref{fig:qualitative_mlat} provide qualitative comparison of all three methods.
...
Iterative multilateration exhibits catastrophic errors scattered throughout the network, illustrating the vulnerability of anchor promotion to error propagation in terrain-rich environments.
...
Table~\ref{tab:freespace_results} shows a striking inversion of performance rankings.
...
When the geometric model is correctly specified (free-space path loss matches reality), direct optimization outperforms learning.
...
Early localization errors in promoted nodes compound through subsequent iterations, explaining the catastrophic outliers in its error distribution.
...
The physics-aware module provides a structured inductive bias that accelerates learning and improves generalization.
...
\paragraph{Post-Installation Calibration}
After deploying beacons, collect anchor-to-anchor RSSI measurements where true distances are known.
Perform lightweight calibration by fine-tuning only the path-loss module parameters ($n$, $o$) while keeping GCN weights frozen.
This aligns the physical model with actual hardware and local conditions without requiring full retraining.
...
\begin{enumerate}
    ...
    \item \textbf{Terrain-Aware Evaluation:} Demonstration that evaluation regime dramatically affects method rankings, with our approach achieving 90\% error reduction over multilateration in realistic conditions while multilateration wins in idealized free-space.
\end{enumerate}
...
This represents substantial improvement over both classical multilateration and plain GCN approaches, establishing that physics-grounded graph learning provides a viable foundation for infrastructure localization when GNSS is unavailable.
...
The preceding chapters have addressed the two core algorithmic problems independently: Chapter~\ref{chap:uav_localization} developed robust UAV localization given known beacon positions, while Chapter~\ref{chap:infrastructure} developed infrastructure self-localization using graph learning.
...
A grid-based RSSI interpolation model is constructed from simulated measurements, enabling evaluation of UAV localization at any position within the deployment area.
The UAV operates at a fixed altitude of 100~m, and the multilateration algorithm from Chapter~\ref{chap:uav_localization} (Huber loss with altitude constraint) is applied using the $k=10$ nearest beacons.
...
\begin{itemize}
    \item \textbf{Localization Error:} Euclidean distance between estimated and true UAV positions, reported as mean, median, and 90th percentile (P90)
    \item \textbf{Waypoint Success Rate:} Percentage of waypoints successfully reached during navigation missions
    \item \textbf{Heading Error:} Angular deviation between the commanded heading (based on estimated position) and the true heading to target
\end{itemize}
...
To obtain statistically robust results, we sample 2,000 random positions within the deployment area and compute localization error at each position for varying levels of beacon position uncertainty.
This grid-wide analysis captures the full distribution of errors across the operational area, avoiding bias from specific flight paths.

Table~\ref{tab:sensitivity_results} presents the localization error statistics for beacon position errors ranging from 0~m (perfect knowledge) to 300~m (beyond expected GCN performance).

\begin{table}[htbp]
\centering
\caption{UAV localization error versus beacon position error}
\label{tab:sensitivity_results}
\begin{tabular}{@{}rrrr@{}}
\toprule
\textbf{Beacon Error (m)} & \textbf{Mean (m)} & \textbf{Median (m)} & \textbf{P90 (m)} \\
\midrule
0 (baseline) & 122 & 115 & 182 \\
50 & 119 & 110 & 193 \\
100 & 122 & 110 & 210 \\
150 & 125 & 112 & 220 \\
200 & 157 & 134 & 287 \\
...
\begin{figure}[htbp]
    ...
    \caption[UAV localization error vs beacon position error]{UAV localization error as a function of beacon position error. The vertical dashed line indicates the GCN self-localization error (206~m). Error bars show standard deviation.}
    \label{fig:sensitivity_main}
\end{figure}

Figure~\ref{fig:sensitivity_main} visualizes the relationship between beacon and UAV errors.
Several observations emerge:

\paragraph{Sub-Linear Degradation}
UAV localization error increases sub-linearly with beacon position error.
At 200~m beacon error (approximately matching GCN performance), UAV error increases from 122~m to 157~m, a degradation of only 29\%.
This sub-linear relationship indicates that the multilateration algorithm provides inherent robustness to beacon position uncertainty through geometric averaging across multiple beacons.
...
This graceful degradation is essential for operational reliability.

\paragraph{Tail Behavior}
Figure~\ref{fig:sensitivity_p90} shows the 90th percentile (P90) error, representing worst-case performance.
At 200~m beacon error, P90 reaches 287~m, indicating that 90\% of positions achieve better than this accuracy.
The tail grows faster than the mean, suggesting that beacon errors more severely impact geometrically unfavorable positions.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\columnwidth]{images/sensitivity_p90.png}
    \caption[90th percentile UAV error vs beacon position error]{90th percentile UAV localization error versus beacon position error, representing worst-case performance across the operational area.}
    \label{fig:sensitivity_p90}
\end{figure}
...
Interpolating from Table~\ref{tab:sensitivity_results}, this corresponds to expected UAV localization error of approximately 160~m mean and 290~m P90.

While this accuracy falls short of GNSS standards (typically 2--5~m), it remains useful for many operational scenarios.
...
To validate the theoretical stability analysis, we conduct closed-loop navigation simulations across three mission profiles representative of common UAV operations.

\subsection{Simulation Setup}

\paragraph{Control Model}
The UAV is modeled with simple proportional control: at each time step, it computes the heading to the current waypoint based on its estimated position and adjusts velocity accordingly.
The UAV flies at 15~m/s with position updates at 1~Hz.
This simplified model captures the essential feedback dynamics without unnecessary complexity.

\paragraph{Waypoint Threshold}
Waypoints are considered reached when the UAV's true position is within 200~m, consistent with the stability analysis.
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.9\columnwidth]{images/diagonal_beacon0m_path.png}
    \caption[Diagonal transit with perfect beacon positions]{Diagonal transit with perfect beacon positions. The true path (blue) closely follows the direct route while the estimated path (red dashed) shows localization noise.}
    \label{fig:diagonal_0m}
\end{figure}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.9\columnwidth]{images/diagonal_beacon200m_path.png}
    \caption[Diagonal transit with 200 m beacon position error]{Diagonal transit with 200~m beacon position error. The true path remains stable during the long transit but shows increased deviation near the destination, illustrating the geometric stability principle.}
    \label{fig:diagonal_200m}
\end{figure}

Figures~\ref{fig:diagonal_0m} and~\ref{fig:diagonal_200m} compare flight paths with perfect and degraded beacon positions.
Figure~\ref{fig:diagonal_200m} provides striking visual confirmation of the geometric stability analysis: during the long transit phase, the true path (blue line) remains nearly straight despite the noisy estimated path (red dashed line).
However, near the destination waypoint, the path shows increased deviation as the geometric stability breaks down at close range.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.9\columnwidth]{images/diagonal_beacon200m_error_timeline.png}
    \caption[Localization error during diagonal transit with 200 m beacon error]{Localization error during diagonal transit with 200~m beacon error. Error varies along the path but remains bounded.}
    \label{fig:diagonal_200m_timeline}
\end{figure}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\columnwidth]{images/diagonal_beacon200m_error_dist.png}
    \caption[Error distribution during diagonal transit]{Distribution of localization errors during diagonal transit with 200~m beacon error.}
    \label{fig:diagonal_200m_dist}
\end{figure}

Figure~\ref{fig:diagonal_200m_timeline} shows the localization error throughout the flight, while Figure~\ref{fig:diagonal_200m_dist} shows the error distribution.
The mission achieves 100\% waypoint success with mean localization error of 152~m.
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.9\columnwidth]{images/circular_beacon0m_path.png}
    \caption[Circular patrol with perfect beacon positions]{Circular patrol with perfect beacon positions. The UAV successfully navigates around all waypoints.}
    \label{fig:circular_0m}
\end{figure}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.9\columnwidth]{images/circular_beacon200m_path.png}
    \caption[Circular patrol with 200 m beacon position error]{Circular patrol with 200~m beacon position error. Despite increased path deviation, the UAV completes all waypoints.}
    \label{fig:circular_200m}
\end{figure}

Figures~\ref{fig:circular_0m} and~\ref{fig:circular_200m} show the patrol paths.
The circular pattern provides consistently moderate distances between waypoints (approximately 780~m), keeping the UAV in the stable navigation regime throughout the mission.
With 200~m beacon error, all 8 waypoints are reached with mean localization error of 137~m.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.9\columnwidth]{images/circular_beacon200m_error_timeline.png}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\columnwidth]{images/circular_beacon200m_error_dist.png}
    \caption[Error distribution during circular patrol]{Distribution of localization errors during circular patrol with 200~m beacon error.}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.9\columnwidth]{images/area_sweep_beacon0m_path.png}
    \caption[Area sweep with perfect beacon positions]{Area sweep with perfect beacon positions. The UAV follows the planned zigzag pattern.}
    \label{fig:sweep_0m}
\end{figure}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.9\columnwidth]{images/area_sweep_beacon200m_path.png}
    \caption[Area sweep with 200 m beacon position error]{Area sweep with 200~m beacon position error. The pattern is maintained with increased track deviation.}
    \label{fig:sweep_200m}
\end{figure}

Figures~\ref{fig:sweep_0m} and~\ref{fig:sweep_200m} show the coverage patterns.
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.9\columnwidth]{images/area_sweep_beacon200m_error_timeline.png}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\columnwidth]{images/area_sweep_beacon200m_error_dist.png}
    \caption[Error distribution during area sweep]{Distribution of localization errors during area sweep with 200~m beacon error.}
    \label{fig:sweep_200m_dist}
\end{figure}

\paragraph{Coverage Implications}
For area coverage missions, the track deviation caused by beacon errors can be compensated by planning overlapping flight paths.
With expected lateral deviation of approximately 150~m at 200~m beacon error, planning overlapping tracks (e.g., 100~m spacing for a 200~m sensor swath, yielding 50\% overlap) ensures complete ground coverage despite navigation uncertainty.
This strategy trades flight efficiency for guaranteed coverage.

\subsection{Comparative Analysis}

Table~\ref{tab:nav_results} summarizes navigation performance across all mission profiles and beacon error levels.

\begin{table}[htbp]
\centering
\caption{Navigation simulation results}
\label{tab:nav_results}
\begin{tabular}{@{}llrr@{}}
\toprule
\textbf{Mission} & \textbf{Beacon Error} & \textbf{Success} & \textbf{Mean Error} \\
\midrule
\multirow{3}{*}{Diagonal} & 0~m & 100\% & 112~m \\
 & 150~m & 100\% & 144~m \\
 & 200~m & 100\% & 152~m \\
\midrule
\multirow{3}{*}{Circular} & 0~m & 100\% & 124~m \\
 & 150~m & 100\% & 118~m \\
 & 200~m & 100\% & 137~m \\
\midrule
\multirow{3}{*}{Area Sweep} & 0~m & 100\% & 115~m \\
 & 150~m & 100\% & 127~m \\
 & 200~m & 100\% & 145~m \\
\bottomrule
\end{tabular}
...
All three mission profiles achieve 100\% waypoint success at beacon error levels up to 200~m, corresponding to expected GCN self-localization performance.
...
A key insight enabling navigation despite high localization error is \textit{geometric stability}: the relationship between position error and navigation error depends strongly on distance to target.
...
The worst-case heading error $\Delta\theta$ occurs when the position error is perpendicular to the true heading direction, and depends on both the position error magnitude $\epsilon$ and the distance to target $d$:
\begin{equation}
    \Delta\theta_{\max} \approx \arctan\left(\frac{\epsilon}{d}\right)
    \label{eq:heading_error}
\end{equation}
When the position error is parallel to the heading direction (toward or away from the target), the heading error approaches zero.

Figure~\ref{fig:geometric_stability} illustrates this worst-case relationship for various position error magnitudes.
Heading error decreases rapidly with distance: at 1~km distance, even 200~m position error produces at most 11 degrees of heading error.
This explains why the diagonal transit in Figure~\ref{fig:diagonal_200m} maintains a stable, nearly straight path during the long transit phase, but shows increased deviation near the destination where the geometric stability breaks down.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\columnwidth]{images/geometric_stability.png}
    \caption[Geometric stability: heading error vs distance]{Geometric stability: worst-case heading error (position error perpendicular to heading) decreases rapidly with distance to target. At 1~km distance, 200~m position error produces at most 11 degrees of heading error.}
    \label{fig:geometric_stability}
\end{figure}

This analysis motivates the 200~m waypoint threshold used in the simulations: by declaring arrival before entering the unstable close-approach regime, the system avoids erratic circling behavior while maintaining mission reliability.

\subsection{Operational Envelope}

The analysis establishes the operational envelope for the integrated system:
...
\begin{itemize}
    \item \textbf{Smaller threshold (100~m):} Higher accuracy when successful, but increased risk of failure due to instability in the close-approach regime
...
The key advantage is resilience: while GNSS can be denied by jamming or environmental obstruction, the proposed system operates independently of external infrastructure.
...
Real-world flight would introduce additional factors including wind, control latency, and sensor noise beyond RSSI variance.

\paragraph{Static Environment}
The analysis assumes a static environment with fixed beacon positions.
In practice, beacon positions might drift due to equipment settling or environmental changes, requiring periodic recalibration.
...
\begin{enumerate}
    \item \textbf{Sub-Linear Error Propagation:} Beacon position errors propagate sub-linearly to UAV localization errors.
    At 200~m beacon error (matching expected GCN performance), UAV error increases by only 29\% from baseline, demonstrating inherent robustness.
    
    \item \textbf{Geometric Stability:} Navigation remains stable when targets are distant.
    For targets beyond 1~km, position errors up to 200~m produce heading errors below 11 degrees, enabling reliable waypoint navigation.
    
    \item \textbf{Mission Feasibility:} Closed-loop simulations demonstrate 100\% waypoint success across diagonal transit, circular patrol, and area sweep missions at beacon error levels up to 200~m.
...
These results establish that despite localization errors substantially larger than GNSS, the integrated system provides operationally useful positioning for emergency and backup navigation scenarios.
...
We addressed two fundamental challenges: client-side UAV localization given beacon positions, and infrastructure self-localization when beacon positions are unknown.
...
Chapter~\ref{chap:uav_localization} presented an enhanced multilateration algorithm that combines the Huber loss function with altitude constraints.
...
Chapter~\ref{chap:infrastructure} developed a Graph Convolutional Network (GCN) architecture that embeds radio propagation physics directly into the learning process.
The key innovation is a trainable RSSI-to-distance module that learns path-loss parameters jointly with the graph message-passing weights.
This physics-aware design enables the network to adapt to local propagation characteristics while maintaining interpretability.
...
Chapter~\ref{chap:integration} analyzed the complete system from infrastructure self-organization through UAV mission execution.
The key finding is that geometric stability enables useful navigation despite substantial localization errors:
\begin{itemize}
    \item Beacon position errors propagate sub-linearly to UAV localization (29\% degradation at 200~m beacon error)
    \item For targets beyond 1~km, position errors up to 200~m produce heading errors below 11 degrees
    \item Closed-loop simulations demonstrate 100\% waypoint success across diverse mission profiles
...
Together, these contributions demonstrate that while LoRa-based positioning cannot match GNSS precision, it provides a viable backup system for emergency operations.
...
The sensitivity of localization accuracy to anchor quantity and geometry was not comprehensively studied.
...
Techniques from domain adaptation and transfer learning could facilitate this simulation-to-real transfer with limited real data.
...
The current system treats each UAV independently.
In multi-UAV operations, vehicles could share position estimates or RSSI observations to improve collective localization accuracy.
Cooperative localization algorithms could exploit inter-UAV measurements when vehicles are within communication range.
...
The increasing reliance on GNSS for autonomous systems creates a critical vulnerability that this thesis has sought to address.
...
The combination of self-organizing infrastructure and robust client-side algorithms creates a system that can be rapidly deployed and operated with minimal prior knowledge of the environment.
