...
In this thesis, we demonstrate that a carefully designed system architecture, combined with targeted algorithmic improvements at both the client and infrastructure layers, can provide operationally useful positioning in GNSS-denied environments despite the inherent limitations of RSSI-based localization.
...
\begin{enumerate}
    ...
    We present a comprehensive feasibility analysis of the integrated system.
    We conduct a sensitivity analysis to quantify how infrastructure estimation errors propagate to client-side localization.
    Furthermore, through closed-loop navigation simulations, we demonstrate that despite localization errors on the order of 150--200~m, the system supports autonomous navigation, achieving 100\% waypoint arrival in tested scenarios.
\end{enumerate}
...
\begin{figure}[htbp]
    ...
    \end{tabular}
    \caption[Static error: Method 4 (proposed: Huber loss with altitude constraint)]{Static localization error for Method 4 (proposed: Huber loss with altitude constraint). (a) Full scale showing occasional large errors; (b) cropped view showing the majority of results achieving 4.5~m median.}
...
%===================================================================
\chapter{Infrastructure Self-Localization via Physics-Aware GCN}
...
Jamming resistance & Low & Configurable \\
...
Regarding jamming resistance, GNSS uses fixed, publicly known frequencies that are easily targeted.
The proposed system can use secret, dynamically changeable operating parameters (frequency, spreading factor, bandwidth) and encryption, providing resilience not available with GNSS.
...
The sensitivity of localization accuracy to anchor quantity and geometry is not comprehensively studied.
