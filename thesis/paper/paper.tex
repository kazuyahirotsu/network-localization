...
Based on the work conducted and published in~\cite{hirotsu2024lora} during my master's program, we present an enhanced multilateration algorithm that combines the Huber loss function with altitude constraints to achieve robust positioning despite the inherent RSSI variance.
...
The threshold $\delta$ determines which residuals are treated as outliers.
We set $\delta = 100$~m, larger than the median RSSI-derived distance error but smaller than typical outlier errors caused by severe multipath or NLOS conditions.
