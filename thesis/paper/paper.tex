...
% Graphics path
% Note: this file is compiled from `thesis/paper/`, while figures live in `thesis/images/`.
% We keep both search styles working:
% - \includegraphics{images/foo}  -> resolved via {../}
% - \includegraphics{foo}         -> resolved via {../images/}
...
% Student ID
\newcommand{\studentid}{37-236431}

% Customize title page: remove "Submitted to..." block and "Professor of Engineering"
\makeatletter
\renewcommand\ist@submittedtoblock{}
\renewcommand\@supervisortitleline{}
% Redefine maketitle to include student ID
\renewcommand\ist@maketitle{\begin{ist@titlepage}%
  ...
  \centering\isttitlesize
    \resizebox{\hsize}{!}{\ist@hookcr\@etitle}\par
    {\@jtitle}\par
    \vskip 20\p@
    by\par
    \vskip 10\p@
    {\studentid\\[3\p@]\@eauthor\\\@jauthor}\par
    \vskip 30\p@
    \ifist@interim
    ...
    \fi
    \vskip 80\p@
    {Supervisor: Prof.\ \@esupervisor \quad \@jsupervisor}\par
    \vskip 20\p@
...
%-------------------
\begin{acknowledge}
I would like to express my deepest gratitude to my supervisor, Professor Akihiro Nakao, for his continuous guidance and encouragement throughout my graduate studies. Since my senior year of undergraduate studies, Professor Nakao has generously dedicated time for weekly meetings, offering insightful feedback that shaped not only the direction of this research but also my growth as a researcher. His dedication and work ethic have been a constant source of inspiration, and our regular discussions helped me maintain steady progress even during the more challenging phases of this work.

I am also grateful to Professor Fabrizio Granelli at the University of Trento, where I had the opportunity to conduct research as an exchange student upon Professor Nakao's recommendation. Professor Granelli's constructive feedback, his assistance in arranging the necessary hardware and organizing the field experiments, made it possible for me to achieve results within a short period.

My appreciation extends to the faculty members and staff of the Nakao Laboratory for providing a supportive research environment, and to my fellow lab members for the many helpful discussions.

Finally, I am deeply thankful to my family and friends for their constant support, and to Margaux for her patience, encouragement, and daily support throughout my studies.
\end{acknowledge}
...
In practice, a beacon network may be deployed in a remote area without prior infrastructure.
...
The purpose of this thesis is to propose and evaluate an integrated positioning system that enables UAV navigation in GNSS-denied environments using commodity LoRa infrastructure.

Specifically, this thesis aims to:
\begin{enumerate}
    \item Propose a robust client-side localization algorithm that achieves reliable UAV positioning using only RSSI measurements, despite the high variance inherent in outdoor RF propagation.
    \item Propose a Graph Convolutional Network (GCN) for infrastructure self-localization that allows a sparse beacon network to determine its own geometry from peer-to-peer RSSI measurements, using only a small subset of anchor nodes with known positions.
    \item Evaluate the feasibility of the integrated system for autonomous UAV navigation, quantifying how infrastructure estimation errors propagate to client-side localization and assessing navigation performance under realistic conditions.
\end{enumerate}
...
To achieve these objectives, this thesis proposes two complementary methods that together form an integrated positioning system for GNSS-denied environments.

\subsection{Client-Side Localization: Robust RSSI Multilateration}

The client-side method estimates the UAV position using RSSI measurements from beacons at known locations.
Standard multilateration finds the position that best fits the set of RSSI-derived distances to multiple beacons, but performs poorly when outlier measurements are present.
We address this challenge through two key modifications.

First, we incorporate altitude constraints derived from onboard barometers or laser altimeters.
By fixing the vertical coordinate to the sensor-derived value, we estimate only the horizontal position $(X, Y)$.
This reduces the problem from three dimensions to two and prevents vertical estimation errors from corrupting the horizontal solution.

Second, we replace the conventional squared error loss with the Huber loss function.
Squared loss penalizes large errors quadratically, causing outliers from multipath fading or environmental shadowing to dominate the solution.
The Huber loss instead applies a linear penalty beyond a threshold, limiting the influence of extreme RSSI values while maintaining sensitivity to small errors.

\subsection{Infrastructure Self-Localization: Physics-Aware Graph Convolutional Network}

The infrastructure-side method addresses the self-localization of the beacon network when only a small subset of nodes (anchors) have known positions.
We formulate this as a graph-based learning problem, where nodes represent beacons and edges represent communication links weighted by RSSI measurements.

We propose a physics-aware Graph Convolutional Network (GCN) that differs from standard graph neural network approaches in two key ways.
First, rather than treating the RSSI-to-position mapping as a black box, our architecture embeds a trainable log-distance path-loss model directly into the network.
The path-loss parameters are learned jointly with the graph neural network weights, allowing the model to adapt to the propagation characteristics of the deployment environment while maintaining physical interpretability.

Second, we employ edge-conditioned convolutions where a small neural network dynamically generates unique message-passing weights for each edge based on its RSSI features and learned distance estimate.
This enables the network to learn which links provide reliable geometric information and which are corrupted by terrain effects, effectively down-weighting unreliable measurements during the localization process.
...
\begin{enumerate}
    ...
    Furthermore, through closed-loop navigation simulations, we demonstrate that despite mean UAV localization errors of approximately 120--160~m when using beacon positions with 200~m mean error, the system supports autonomous navigation, achieving 100\% waypoint arrival (with a 200~m waypoint threshold) in tested scenarios.
\end{enumerate}
...
\textbf{Chapter~\ref{chap:uav_localization}: Client-Side Localization} addresses robust UAV positioning using RSSI from beacons at known locations.
...
\textbf{Chapter~\ref{chap:infrastructure}: Infrastructure-Side Localization} addresses beacon network self-localization when only a subset of nodes have known positions.
We formulate the task as a graph-based problem, present the physics-aware GCN architecture, and evaluate performance against baselines using terrain-aware Longley-Rice simulations.
...
Wireless positioning systems estimate location by analyzing signals exchanged between a target device and reference nodes at known positions~\cite{zafari2019survey}.
...
However, RSSI exhibits high variance due to multipath propagation, shadowing from obstacles, antenna orientation, and environmental factors~\cite{goldoni2019lora}.
...
Time-of-Flight (ToF) measures the propagation delay of a signal traveling from transmitter to receiver, converting time to distance using the speed of light~\cite{zafari2019survey}.
...
AoA methods determine the direction from which a signal arrives using antenna arrays, enabling triangulation-based positioning~\cite{zafari2019survey}.
...
Simultaneous Localization and Mapping (SLAM) uses camera imagery to construct an environmental map while simultaneously tracking the camera's position within that map~\cite{murartal2017orbslam2}.
...
In this model, beacons broadcast their positions, and UAVs passively receive these broadcasts without transmitting, as shown in Figure~\ref{fig:system_architecture}.
...
%===================================================================
% CHAPTER 4: CLIENT-SIDE LOCALIZATION
%===================================================================
\chapter{Client-Side Localization: Robust Multilateration for UAV Positioning}
\label{chap:uav_localization}

The system architecture presented in Chapter~\ref{chap:architecture} enables UAV positioning through beacon broadcasts received by the client.
However, even when beacon positions are known, achieving accurate UAV localization from RSSI measurements is challenging due to the high signal variance caused by multipath fading and environmental factors.
This chapter addresses this client-side problem.

Based on the work conducted during a research stay at the University of Trento, Italy, in collaboration with Prof. Fabrizio Granelli, and published in~\cite{hirotsu2024lora}, we present an enhanced multilateration algorithm that combines the Huber loss function with altitude constraints to achieve robust positioning despite the inherent RSSI variance.
...
We consider a UAV operating in an environment where GNSS signals are unavailable or unreliable.
...
The parameters $n$ and $o$ are determined through least-squares regression on calibration measurements collected in controlled conditions. Section~\ref{sec:uav_simulation} details the calibration procedure.
...
Squared loss is optimal when errors follow a Gaussian distribution, but it performs poorly when the error distribution includes occasional large deviations.
Large residuals receive disproportionate weight, which can pull the estimate away from the true location.
In a scenario where one residual is 100~m and ten residuals are 10~m, the single large residual contributes $100^2 = 10000$ to the objective while the ten smaller residuals contribute $10 \times 10^2 = 1000$ in total.
The single outlier therefore outweighs ten consistent measurements by a factor of ten, allowing one corrupted RSSI measurement to dominate the solution.
...
This property aligns with the physical characteristics of RSSI measurements in outdoor environments.
RSSI values naturally separate into two groups.
Line-of-sight (LOS) links, where signals travel directly between transmitter and receiver, follow the expected path-loss model with moderate noise and produce small distance estimation errors.
Non-line-of-sight (NLOS) links, where signals are obstructed and arrive via reflection or diffraction, exhibit severe attenuation that the path-loss model cannot account for, resulting in large distance overestimates.
By applying quadratic penalty to small residuals and linear penalty to large residuals, the Huber loss preserves the accuracy of LOS measurements while limiting the influence of NLOS outliers on the position estimate.

The threshold $\delta$ determines the boundary between these two cases.
We set $\delta = 100$~m because LOS links typically produce distance errors below this value, while NLOS links often exceed it.
...
\begin{figure}[htbp]
    \centering
    \begin{tabular}{cc}
    \includegraphics[width=0.48\columnwidth]{images/method1_2d.pdf} &
        \includegraphics[width=0.48\columnwidth]{images/method1_3d.pdf} \\
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\columnwidth]{images/method2.pdf}
...
\begin{figure}[htbp]
    \centering
    \begin{tabular}{cc}
        \includegraphics[width=0.48\columnwidth]{images/method3_2d.pdf} &
        \includegraphics[width=0.48\columnwidth]{images/method3_3d.pdf} \\
        (a) 2D error & (b) 3D error
    \end{tabular}
...
\begin{figure}[htbp]
    \centering
    \begin{tabular}{cc}
        \includegraphics[width=0.48\columnwidth]{images/method4.pdf} &
        \includegraphics[width=0.48\columnwidth]{images/method4_cropped.pdf} \\
...
Figure~\ref{fig:static_method4}(a) shows occasional large errors unique to Method 4; these outliers are addressed through error monitoring as discussed below.

\subsubsection{Scalability Analysis}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.9\columnwidth]{method_comparison.pdf}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\columnwidth]{correlation.pdf}
...
Figure~\ref{fig:experiment_site} shows the experiment site with the UAV platform.
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.9\columnwidth]{images/site.jpg}
    \caption[Experiment site]{Experiment site in Pergine, Italy.}
    \label{fig:experiment_site}
\end{figure}
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[b]{0.45\columnwidth}
        ...
        \label{fig:beacon}
    \end{subfigure}
    \caption[Experimental hardware]{Experimental hardware used in the field deployment.}
...
Field experiments with a larger number of beacons would better demonstrate the scalability advantage of Huber loss observed in simulation and remain as future work.
...
\begin{figure}[htbp]
    \centering
    \begin{tabular}{cc}
        \includegraphics[width=0.48\columnwidth]{model_1-2.pdf} &
        \includegraphics[width=0.48\columnwidth]{model_3-4.pdf} \\
...
The dashed line shows the free-space path loss model, which assumes isotropic antennas with uniform gain in all directions and depends only on distance.
The colored curves incorporate the monopole's elevation-angle-dependent radiation pattern from Equation~\ref{eq:antenna_gain}.
At short distances around 20~m, the high-altitude measurements in green show increasing RSSI as distance increases, opposite to the expected behavior.
...
In contrast, low-altitude measurements in blue more closely follow the free-space model, since the elevation angle remains close to the horizon where the antenna gain is near maximum.
This deviation illustrates why the simple log-distance path loss model fails when antenna directivity is significant.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.7\columnwidth]{antenna.pdf}
    \caption[Signal strength variation with UAV altitude]{Signal strength versus distance at different UAV altitudes compared to free-space path loss (dashed line). Antenna directivity causes deviation from the path loss model at high altitudes.}
...
\begin{enumerate}
    \item \textbf{Directivity-Aware RSSI Model:} The standard log-distance path loss model cannot capture antenna directivity effects even when parameterized by altitude, because the radiation pattern follows a non-logarithmic function (Equation~\ref{eq:antenna_gain}).
    Instead, the model must explicitly incorporate the elevation-angle-dependent antenna gain:
    \begin{equation}
        d_i = f(\text{RSSI}_i, Z_{\text{UAV}}, Z_{\text{beacon}}, E(\theta))
    \end{equation}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\columnwidth]{images/model_3d.pdf}
...
\begin{figure}[htbp]
    \centering
    \begin{tabular}{cc}
        \includegraphics[width=0.48\columnwidth]{sample-median.pdf} &
        \includegraphics[width=0.48\columnwidth]{sample-std.pdf} \\
...
\subsection{Altitude Sensor Accuracy}

The altitude-constrained multilateration relies on barometric (1--5~m accuracy) or laser altimeter (centimeter-level) readings.
These uncertainties are negligible compared to RSSI-induced errors, where shadow fading translates to distance errors of tens to hundreds of meters.
Environmental effects on RSSI, not altitude sensor accuracy, are the dominant source of localization error.
...
%===================================================================
% CHAPTER 5: INFRASTRUCTURE-SIDE LOCALIZATION
%===================================================================
\chapter{Infrastructure-Side Localization: Physics-Aware GCN for Beacon Networks}
\label{chap:infrastructure}

The client-side localization method presented in Chapter~\ref{chap:uav_localization} assumes that beacon positions are known.
However, in GNSS-denied environments, establishing these beacon positions is itself a significant challenge.
Before the system can provide positioning services to UAV clients, the beacon network must first determine its own geometry.
This chapter addresses this infrastructure-side problem.

Unlike the client-side problem where beacon positions are given, here we must infer beacon positions from pairwise RSSI measurements within the beacon network, with only a small subset of nodes having known coordinates.
This problem is challenging because anchors are sparse and ground-to-ground links are heavily affected by terrain.
Classical approaches such as iterative multilateration fail in this setting because they treat all links equally and allow early estimation errors to compound through subsequent iterations.

To address these challenges, we propose a physics-aware Graph Convolutional Network (GCN) that learns to localize beacons by combining a trainable RSSI-to-distance module with graph-based message passing.
The network uses edge-conditioned convolutions that adapt per link, allowing the model to learn how much to trust each measurement rather than treating all links equally.
...
We consider a network of $N$ static beacons deployed in an outdoor area where GNSS is unavailable or unreliable.
...
Note that terrain elevation data is not provided as input to the model.
Terrain effects enter only through their influence on the RSSI measurements that form the edge features. Links obstructed by ridges or shadowed by hills exhibit stronger attenuation and higher variance than line-of-sight links.
The GCN learns to interpret these terrain-induced statistical patterns through edge-conditioned message passing, effectively extracting terrain information from the observed RSSI patterns rather than requiring explicit terrain input.
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=\columnwidth]{images/experiment_area_comparison.pdf}
    ...
    \begin{minipage}[t]{0.5\columnwidth}
        ...
        (b) Elevation map
    \end{minipage}
    \caption[Evaluation area]{Evaluation area showing terrain complexity with 4~km $\times$ 4~km extent.}
...
\begin{figure}[htbp]
    ...
    \begin{minipage}[t]{0.5\columnwidth}
        \centering
        (a) Terrain-aware setting
    \end{minipage}%
    \begin{minipage}[t]{0.5\columnwidth}
        \centering
        (b) Free-space setting
    \end{minipage}
    \caption[RSSI versus distance relationship]{RSSI versus distance relationship. The terrain-aware setting shows high variance and non-linearity, while the free-space setting shows a clean log-distance relationship.}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=\columnwidth]{images/three_methods_cdf.pdf}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=\columnwidth]{images/three_methods_hist.pdf}
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[b]{0.32\columnwidth}
        \centering
        \includegraphics[width=\textwidth]{images/sample_visualization_loaded_model_trained_localization_model_64beacons_1000instances_fixed_power13.pdf}
        \caption{Proposed GCN}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.32\columnwidth}
        \centering
        \includegraphics[width=\textwidth]{images/sample_visualization_loaded_model_trained_localization_model_64beacons_1000instances_fixed_no_rssi2dist.pdf}
        \caption{Plain GCN}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.32\columnwidth}
        \centering
        \includegraphics[width=\textwidth]{images/sample_visualization_loaded_model_mlat_64beacons_100instances.pdf}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=\columnwidth]{images/three_methods_cdf_free.pdf}
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[b]{0.32\columnwidth}
        \centering
        \includegraphics[width=\textwidth]{images/sample_visualization_loaded_model_trained_localization_model_64beacons_1000instances_fixed_power13_free.pdf}
        \caption{Proposed GCN}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.32\columnwidth}
        \centering
        \includegraphics[width=\textwidth]{images/sample_visualization_loaded_model_trained_localization_model_64beacons_1000instances_fixed_no_rssi2dist_free.pdf}
        \caption{Plain GCN}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.32\columnwidth}
        \centering
        \includegraphics[width=\textwidth]{images/sample_visualization_loaded_model_mlat_64beacons_1000instances_free.pdf}
        \caption{Multilateration}
    \end{subfigure}
...
Manufacturing tolerances cause slight differences between beacons, such as 1--2~dB variation in transmission power.
However, as shown in Figure~\ref{fig:rssi_variance}, terrain-induced RSSI variance spans approximately 70--80~dB for a given distance, making hardware variations negligible by comparison.
The dominant error source remains terrain-induced propagation effects, not hardware differences between beacons.
...
Our simulations use grid-constrained anchor placement and uniformly random unknown node placement without optimizing deployment strategy.
In practice, beacon placement relative to terrain strongly influences localization performance, because line-of-sight (LOS) links produce RSSI values that follow the path-loss model more closely, yielding more accurate distance estimates, while NLOS links exhibit severe attenuation and high variance that degrade distance accuracy.
Maximizing the proportion of LOS links in the network graph is therefore a key deployment consideration.

In mountainous terrain, beacons on hilltops or ridges maintain LOS connections to more neighbors, while beacons in valleys connect only to nearby nodes on the same valley floor, creating regions of sparse connectivity.
Although the edge-conditioned convolutions can learn to down-weight unreliable NLOS links, this mitigation is insufficient when most links to a node are NLOS, as the model lacks reliable geometric information regardless of edge weighting.
A practical strategy would prioritize elevated positions and, when valley coverage is needed, place additional beacons on surrounding ridgelines to bridge poorly connected regions.

For anchors specifically, a relevant scenario is placing them only around the perimeter of a GNSS-denied region, which differs from our distributed grid layout and may exhibit different error characteristics for interior nodes far from all anchors.
Systematic investigation of placement optimization under terrain and accessibility constraints remains a direction for future work.
...
The model captures terrain effects implicitly through RSSI statistics rather than receiving terrain data as explicit input, so the learned propagation patterns are specific to the terrain used for training.
Deploying in a new region requires regenerating training data using terrain-aware propagation simulation for that region and retraining the model, because each terrain produces distinct propagation characteristics that the model cannot generalize from one region to another.
A promising direction for future work is developing a foundation model pre-trained on diverse terrain conditions, which could then be fine-tuned for a new deployment environment without full retraining.
Fine-tuning could use a small amount of region-specific simulation data, or anchor-to-anchor RSSI measurements collected after physical installation.
The latter is particularly promising because anchor positions are known, providing ground-truth distance-RSSI pairs that can also calibrate the discrepancies between the propagation simulator and actual conditions.

\paragraph{Computational Cost}
Generating terrain-specific training data requires running propagation simulations for all pairwise links, and the computational cost grows with area size and number of beacons.
However, each link can be simulated independently, making the process straightforward to parallelize.
In practice, physical beacon deployment (site access, installation, power provisioning) is likely a greater bottleneck than the computational preparation.
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\columnwidth]{images/sensitivity_uav_vs_beacon.pdf}
...
Even with perfect beacon positions, UAV localization exhibits a baseline error of approximately 122~m due to inherent RSSI measurement noise.
This baseline represents the fundamental limitation of RSSI-based multilateration in the simulated terrain environment.
The influence of beacon position error becomes apparent only when it approaches or exceeds this baseline level. Below approximately 150~m beacon error, the curves remain relatively flat.

\paragraph{Moderate Error Propagation}
Beyond the baseline, UAV localization error increases approximately linearly with beacon position error, but at a rate less than 1:1.
At 200~m beacon error (approximately matching GCN performance), mean UAV error reaches 157~m, an increase of only 35~m from the baseline.
This moderate propagation occurs because the multilateration algorithm averages across multiple beacons with independent errors, partially canceling out individual beacon position inaccuracies.
Importantly, the system does not exhibit catastrophic failure modes.
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[b]{0.48\columnwidth}
        \centering
        \includegraphics[width=\textwidth]{images/diagonal_beacon0m_path.pdf}
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[b]{0.48\columnwidth}
        \centering
        \includegraphics[width=\textwidth]{images/diagonal_beacon200m_error_timeline.pdf}
    ...
    \begin{subfigure}[b]{0.48\columnwidth}
        \centering
        \includegraphics[width=\textwidth]{images/diagonal_beacon200m_error_dist.pdf}
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[b]{0.48\columnwidth}
        \centering
        \includegraphics[width=\textwidth]{images/circular_beacon0m_path.pdf}
        \caption{Perfect beacon positions}
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[b]{0.48\columnwidth}
        \centering
        \includegraphics[width=\textwidth]{images/circular_beacon200m_error_timeline.pdf}
        \caption{Error over time}
    ...
    \begin{subfigure}[b]{0.48\columnwidth}
        \centering
        \includegraphics[width=\textwidth]{images/circular_beacon200m_error_dist.pdf}
        \caption{Error distribution}
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[b]{0.48\columnwidth}
        \centering
        \includegraphics[width=\textwidth]{images/area_sweep_beacon0m_path.pdf}
        \caption{Perfect beacon positions}
        \label{fig:sweep_0m}
    \end{subfigure}
    \hfill
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[b]{0.48\columnwidth}
        \centering
        \includegraphics[width=\textwidth]{images/area_sweep_beacon200m_error_timeline.pdf}
        \caption{Error over time}
    ...
    \begin{subfigure}[b]{0.48\columnwidth}
        \centering
        \includegraphics[width=\textwidth]{images/area_sweep_beacon200m_error_dist.pdf}
        \caption{Error distribution}
...
We consider a UAV at true position $\mathbf{p}_{\text{true}}$ with estimated position $\mathbf{p}_{\text{est}}$ attempting to navigate toward target $\mathbf{t}$.
...
The figure shows that heading error decreases rapidly as the target distance increases.
This is why long-range navigation remains stable even with large localization errors.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\columnwidth]{images/geometric_stability.pdf}
...
The antenna directivity effects observed in Chapter~\ref{chap:uav_localization} suggest that varying altitude would require directivity-aware RSSI models not fully developed in this work.
...
\paragraph{Directivity-Aware Models}
The antenna directivity effects observed in field experiments motivate development of directivity-aware RSSI-to-distance models.
Such models would explicitly incorporate the antenna radiation pattern, improving accuracy for UAVs operating at varying altitudes.
