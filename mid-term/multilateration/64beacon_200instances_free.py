...
matlab_data_base_path = "../../matlab/data/64beacons_1000instances_free/" # As per your current selection
output_dir = "output_visualizations_matlab_im_free"
...
GIF_FRAMES_SUBDIR = f"{dataset_name}_{node_config_str}_im_instance_1_gif_frames"
...
if len(all_instance_errors) > 0: # Ensure there are errors to save
    error_save_filename = os.path.join(output_dir, f"{dataset_name}_{node_config_str}_im_simple_errors_raw.npy")
...
if num_instances > 0:
    ...
    if node_pos_true_viz is not None:
        ...
        for iter_num_viz in tqdm(range(num_iterations), desc=f"Generating GIF frames for Instance {instance_to_viz}"):
            ...
            for i_node_viz in range(num_anchors, total_nodes):
            ...
            # Re-enable and make title iteration-specific for frames
            ...
            x_min_limit = np.min(node_pos_true_viz[:,0]) - 500
            x_max_limit = np.max(node_pos_true_viz[:,0]) + 500
            y_min_limit = np.min(node_pos_true_viz[:,1]) - 500
            y_max_limit = np.max(node_pos_true_viz[:,1]) + 500
        ...
        plt.figure(figsize=(12, 10))
        plt.scatter(node_pos_true_viz[num_anchors:, 0], node_pos_true_viz[num_anchors:, 1], c='blue', label='True Unknown Positions', alpha=0.6, s=50)
        plt.scatter(final_estimated_pos_viz[num_anchors:, 0], final_estimated_pos_viz[num_anchors:, 1], c='red', marker='x', label="Predicted Unknown Positions", alpha=0.8, s=50) # Use your preferred label
        plt.scatter(node_pos_true_viz[:num_anchors, 0], node_pos_true_viz[:num_anchors, 1], c='green', marker='^', s=100, label='Anchor Nodes', edgecolors='black')
        ...
        plt.xlabel('X Position (m)', fontsize=SLIDE_AXIS_LABEL_FONT_SIZE)
        plt.ylabel('Y Position (m)', fontsize=SLIDE_AXIS_LABEL_FONT_SIZE)
        # Re-enable title for the final state plot
        ...
        plt.grid(True)
        plt.axis('equal')
        # Match axis limits with GIF frames for consistency
        plt.xlim(x_min_limit, x_max_limit) 
        plt.ylim(y_min_limit, y_max_limit)
        plt.tight_layout()
