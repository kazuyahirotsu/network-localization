...
#######################################
load_path = 'trained_localization_model_36beacons_1000instances_fixed.pth'
...
with torch.no_grad():
    for data in tqdm(test_loader_loaded, desc="Evaluating with loaded model"):
        data = data.to(device)
...
plt.figure(figsize=(12, 6))
    plt.hist(errors_gcn_loaded, bins=20, alpha=0.7, color='lightcoral', label='GCN Errors (Loaded Model)')
plt.xlabel('Localization Error (meters)')
plt.ylabel('Number of Nodes')
    plt.title('Error Distribution for GCN on Test Data (Loaded Model)')
plt.legend()
plt.show()
...
# 
# 9.  **Results:**
#     *   An error histogram and mean/median errors are printed for the loaded model's performance on the test set. An additional check is added to handle cases where `errors_gcn_loaded` might be empty (e.g., if the test set had no unknown nodes).
# 
# **To Run This Script:**
# 
# 1.  Ensure the `trained_localization_model_36beacons_1000instances_fixed.pth` file is in the same directory as this script (or provide the correct path to it).
# 2.  Make sure the data path (`../matlab/data/...`) is correct relative to where you run this script.
