...
#######################################
load_path = 'trained_localization_model_64beacons_1000instances_fixed_power13.pth'
...
errors_gcn_loaded = []
dist_error_pairs = []
dist_nearest_node_error_pairs = []
dist_avg_3_nearest_nodes_error_pairs = []
strongest_rssi_error_pairs = []
avg_3_strongest_rssi_error_pairs = []
...
with torch.no_grad():
    for i, data in enumerate(tqdm(test_loader_loaded, desc="Evaluating with loaded model")):
        ...
        anchor_positions = true_positions[data.anchor_mask.cpu().numpy()]
        for idx in range(true_positions.shape[0]):
            if data.unknown_mask[idx]:
                ...
                if anchor_positions.shape[0] > 0:
                    distances_to_anchors = np.linalg.norm(anchor_positions - true_pos, axis=1)
                    min_dist_to_anchor = np.min(distances_to_anchors)
                    dist_error_pairs.append((min_dist_to_anchor, error))

                # Calculate distance to nearest N nodes (any type)
                all_other_pos = np.delete(true_positions, idx, axis=0)
                distances_to_all_other_nodes = np.linalg.norm(all_other_pos - true_pos, axis=1)

                if distances_to_all_other_nodes.size > 0:
                    distances_to_all_other_nodes.sort()
                    
                    # 1. Distance to the single nearest node
                    dist_to_nearest_node = distances_to_all_other_nodes[0]
                    dist_nearest_node_error_pairs.append((dist_to_nearest_node, error))
                    
                    # 2. Average distance to the 3 nearest nodes
                    if distances_to_all_other_nodes.size >= 3:
                        avg_dist_3_nearest = np.mean(distances_to_all_other_nodes[:3])
                        dist_avg_3_nearest_nodes_error_pairs.append((avg_dist_3_nearest, error))

                # --- RSSI-based Analysis ---
                # Find all edges connecting to the current node 'idx'
                incoming_edge_mask = (data.edge_index[1] == idx)
                if torch.any(incoming_edge_mask):
                    incoming_rssi_means = data.edge_attr[incoming_edge_mask].mean(dim=1).cpu().numpy()
                    incoming_rssi_means.sort() # Sorts in ascending order (weakest to strongest)
                    
                    # 1. Strongest RSSI
                    strongest_rssi = incoming_rssi_means[-1]
                    strongest_rssi_error_pairs.append((strongest_rssi, error))
                    
                    # 2. Average of 3 Strongest RSSI
                    if len(incoming_rssi_means) >= 3:
                        avg_3_strongest_rssi = np.mean(incoming_rssi_means[-3:])
                        avg_3_strongest_rssi_error_pairs.append((avg_3_strongest_rssi, error))
...
if dist_error_pairs:
    distances, errors = zip(*dist_error_pairs)
    correlation, _ = pearsonr(distances, errors)
    print(f"\nCorrelation (Error vs Dist to Nearest Anchor): {correlation:.4f}")
    ...
    for d, e in dist_error_pairs:
        # Find bin index for distance d
        bin_idx = np.searchsorted(bins, d, side="right") - 1
        # Ensure bin_idx is within the valid range [0, num_bins-1]
        bin_idx = max(0, min(bin_idx, num_bins - 1))
        binned_errors[bin_idx].append(e)

    plt.figure(figsize=(15, 8))
    plt.boxplot(binned_errors, labels=bin_labels)
    plt.xlabel('Distance to Nearest Anchor (m)', fontsize=SLIDE_AXIS_LABEL_FONT_SIZE)
    plt.ylabel('Localization Error (m)', fontsize=SLIDE_AXIS_LABEL_FONT_SIZE)
    plt.xticks(fontsize=SLIDE_TICK_LABEL_FONT_SIZE, rotation=45)
    plt.yticks(fontsize=SLIDE_TICK_LABEL_FONT_SIZE)
    plt.grid(True, axis='y', alpha=0.3)
    plt.title(f'Error vs. Distance to Nearest Anchor\nPearson Correlation: {correlation:.4f}', fontsize=SLIDE_TITLE_FONT_SIZE)
    
    dist_error_filename = f'new_results/boxplot_dist_vs_error_{load_path.split("/")[-1].replace(".pth","")}.png'
    plt.tight_layout()
    plt.savefig(dist_error_filename, dpi=300, bbox_inches='tight')
    print(f'Distance vs. Error box plot saved to {dist_error_filename}')
    plt.show()

# --- Plot for Nearest Node Analysis ---
if dist_nearest_node_error_pairs:
    distances, errors = zip(*dist_nearest_node_error_pairs)
    correlation, _ = pearsonr(distances, errors)
    print(f"Correlation (Error vs Dist to Nearest Node): {correlation:.4f}")
    ...
    for d, e in dist_nearest_node_error_pairs:
        bin_idx = np.searchsorted(bins, d, side="right") - 1
        bin_idx = max(0, min(bin_idx, num_bins - 1))
        binned_errors[bin_idx].append(e)
    ...
    plt.figure(figsize=(15, 8))
    plt.boxplot(binned_errors, labels=bin_labels)
    plt.xlabel('Distance to Nearest Node (m)', fontsize=SLIDE_AXIS_LABEL_FONT_SIZE)
    plt.ylabel('Localization Error (m)', fontsize=SLIDE_AXIS_LABEL_FONT_SIZE)
    plt.xticks(fontsize=SLIDE_TICK_LABEL_FONT_SIZE, rotation=45)
    plt.yticks(fontsize=SLIDE_TICK_LABEL_FONT_SIZE)
    plt.grid(True, axis='y', alpha=0.3)
    plt.title(f'Error vs. Distance to Nearest Node\nPearson Correlation: {correlation:.4f}', fontsize=SLIDE_TITLE_FONT_SIZE)

    filename = f'new_results/boxplot_dist_nearest_node_vs_error_{load_path.split("/")[-1].replace(".pth","")}.png'
    plt.tight_layout()
    plt.savefig(filename, dpi=300, bbox_inches='tight')
    print(f'Nearest Node Distance vs. Error box plot saved to {filename}')
    plt.show()

# --- Plot for 3-Nearest Nodes Analysis ---
if dist_avg_3_nearest_nodes_error_pairs:
    distances, errors = zip(*dist_avg_3_nearest_nodes_error_pairs)
    correlation, _ = pearsonr(distances, errors)
    print(f"Correlation (Error vs Avg Dist to 3 Nearest Nodes): {correlation:.4f}")
    ...
    for d, e in dist_avg_3_nearest_nodes_error_pairs:
        bin_idx = np.searchsorted(bins, d, side="right") - 1
        bin_idx = max(0, min(bin_idx, num_bins - 1))
        binned_errors[bin_idx].append(e)
    ...
    plt.figure(figsize=(15, 8))
    plt.boxplot(binned_errors, labels=bin_labels)
    plt.xlabel('Avg. Distance to 3 Nearest Nodes (m)', fontsize=SLIDE_AXIS_LABEL_FONT_SIZE)
    plt.ylabel('Localization Error (m)', fontsize=SLIDE_AXIS_LABEL_FONT_SIZE)
    plt.xticks(fontsize=SLIDE_TICK_LABEL_FONT_SIZE, rotation=45)
    plt.yticks(fontsize=SLIDE_TICK_LABEL_FONT_SIZE)
    plt.grid(True, axis='y', alpha=0.3)
    plt.title(f'Error vs. Avg. Distance to 3 Nearest Nodes\nPearson Correlation: {correlation:.4f}', fontsize=SLIDE_TITLE_FONT_SIZE)

    filename = f'new_results/boxplot_dist_avg_3_nearest_vs_error_{load_path.split("/")[-1].replace(".pth","")}.png'
    plt.tight_layout()
    plt.savefig(filename, dpi=300, bbox_inches='tight')
    print(f'Avg. 3-Nearest-Node Distance vs. Error box plot saved to {filename}')
    plt.show()

# --- Plot for Strongest RSSI Analysis ---
if strongest_rssi_error_pairs:
    rssi_values, errors = zip(*strongest_rssi_error_pairs)
    correlation, _ = pearsonr(rssi_values, errors)
    print(f"Correlation (Error vs Strongest RSSI): {correlation:.4f}")

    # Create box plot (note: RSSI is negative, so bins are handled accordingly)
    num_bins = 10
    bins = np.linspace(np.min(rssi_values), np.max(rssi_values), num_bins + 1)
    bin_labels = [f'{bins[i]:.0f} to {bins[i+1]:.0f}' for i in range(num_bins)]
    binned_errors = [[] for _ in range(num_bins)]

    for r, e in strongest_rssi_error_pairs:
    ...
    plt.figure(figsize=(15, 8))
    plt.boxplot(binned_errors, labels=bin_labels)
    plt.xlabel('Strongest RSSI (dBm)', fontsize=SLIDE_AXIS_LABEL_FONT_SIZE)
    plt.ylabel('Localization Error (m)', fontsize=SLIDE_AXIS_LABEL_FONT_SIZE)
    plt.xticks(fontsize=SLIDE_TICK_LABEL_FONT_SIZE, rotation=45)
    plt.yticks(fontsize=SLIDE_TICK_LABEL_FONT_SIZE)
    plt.grid(True, axis='y', alpha=0.3)
    plt.title(f'Error vs. Strongest RSSI\nPearson Correlation: {correlation:.4f}', fontsize=SLIDE_TITLE_FONT_SIZE)

    filename = f'new_results/boxplot_strongest_rssi_vs_error_{load_path.split("/")[-1].replace(".pth","")}.png'
    plt.tight_layout()
    plt.savefig(filename, dpi=300, bbox_inches='tight')
    print(f'Strongest RSSI vs. Error box plot saved to {filename}')
    plt.show()

# --- Plot for Avg. 3 Strongest RSSI Analysis ---
if avg_3_strongest_rssi_error_pairs:
    rssi_values, errors = zip(*avg_3_strongest_rssi_error_pairs)
    correlation, _ = pearsonr(rssi_values, errors)
    print(f"Correlation (Error vs Avg of 3 Strongest RSSI): {correlation:.4f}")

    # Create box plot
    num_bins = 10
    bins = np.linspace(np.min(rssi_values), np.max(rssi_values), num_bins + 1)
    bin_labels = [f'{bins[i]:.1f} to {bins[i+1]:.1f}' for i in range(num_bins)]
    binned_errors = [[] for _ in range(num_bins)]

    for r, e in avg_3_strongest_rssi_error_pairs:
        bin_idx = np.searchsorted(bins, r, side="right") - 1
        bin_idx = max(0, min(bin_idx, num_bins - 1))
        binned_errors[bin_idx].append(e)
    ...
    plt.figure(figsize=(15, 8))
    plt.boxplot(binned_errors, labels=bin_labels)
    plt.xlabel('Avg. of 3 Strongest RSSI (dBm)', fontsize=SLIDE_AXIS_LABEL_FONT_SIZE)
    plt.ylabel('Localization Error (m)', fontsize=SLIDE_AXIS_LABEL_FONT_SIZE)
    plt.xticks(fontsize=SLIDE_TICK_LABEL_FONT_SIZE, rotation=45)
    plt.yticks(fontsize=SLIDE_TICK_LABEL_FONT_SIZE)
    plt.grid(True, axis='y', alpha=0.3)
    plt.title(f'Error vs. Avg. of 3 Strongest RSSI\nPearson Correlation: {correlation:.4f}', fontsize=SLIDE_TITLE_FONT_SIZE)
    
    filename = f'new_results/boxplot_avg_3_strongest_rssi_vs_error_{load_path.split("/")[-1].replace(".pth","")}.png'
    plt.tight_layout()
    plt.savefig(filename, dpi=300, bbox_inches='tight')
    print(f'Avg. 3 Strongest RSSI vs. Error box plot saved to {filename}')
    plt.show()
