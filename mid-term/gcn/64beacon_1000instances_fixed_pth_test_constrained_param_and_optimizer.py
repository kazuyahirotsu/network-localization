...
#######################################
load_path = 'trained_localization_model_64beacons_1000instances_fixed_constrained_offset_and_optimizer.pth'
...
# -> ADDITION: Initialize list to store true and GCN estimated distances for edges
...
with torch.no_grad():
    for i, data in enumerate(tqdm(test_loader_loaded, desc="Evaluating with loaded model")):
        ...
        # -> ADDITION: Collect true and GCN estimated distances for each edge
...
if errors_gcn_loaded:
    ...
    errors_filename = f'results/gcn_errors_{load_path.split("/")[-1].replace(".pth","")}.npy'
    np.save(errors_filename, errors_gcn_loaded)
    print(f'GCN errors saved to {errors_filename}')

    # -> ADDITION: Save the collected true vs. GCN estimated distances
