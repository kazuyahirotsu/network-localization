        ...
        for idx in range(true_positions.shape[0]):
            if data.unknown_mask[idx]:
                ...
                if distances_to_all_other_nodes.size > 0:
                    ...
                    dist_nearest_node_error_pairs.append((distances_to_all_other_nodes[0], error))
                ...
                if torch.any(incoming_edge_mask):
                    ...
                    incoming_rssi_means.sort()
                    strongest_rssi_error_pairs.append((incoming_rssi_means[-1], error))
...
# --- Plot for Nearest Anchor Analysis ---
if dist_error_pairs:
    ...
    for d, e in dist_error_pairs:
        bin_idx = max(0, min(np.searchsorted(bins, d, side="right") - 1, num_bins - 1))
    ...
    filename = f'new_results/boxplot_dist_vs_error_{load_path.split("/")[-1].replace(".pth","")}.png'
    plt.tight_layout()
    plt.savefig(filename, dpi=300, bbox_inches='tight')
    print(f'Distance vs. Error box plot saved to {filename}')
    plt.show()
...
if strongest_rssi_error_pairs:
    ...
    for r, e in strongest_rssi_error_pairs:
        bin_idx = max(0, min(np.searchsorted(bins, r, side="right") - 1, num_bins - 1))
        binned_errors[bin_idx].append(e)
