...
#######################################
load_path = 'trained_localization_model_64beacons_1000instances_fixed_no_rssi2dist_free.pth'
...
with torch.no_grad():
    for i, data in enumerate(tqdm(test_loader_loaded, desc="Evaluating with loaded model")):
        ...
        if i == 0 and not plotted_sample: # Check plotted_sample to ensure it runs only once if loader is re-iterated
            ...
            sample_viz_filename = f'new_results/sample_visualization_loaded_model_{load_path.split("/")[-1].replace(".pth","")}_no_rssi2dist_free.png'
            plt.tight_layout()
...
if errors_gcn_loaded:
    ...
    metrics_filename = f'new_results/metrics_loaded_model_{load_path.split("/")[-1].replace(".pth","")}_no_rssi2dist_free.txt'
    ...
    error_hist_filename = f'new_results/error_histogram_loaded_model_{load_path.split("/")[-1].replace(".pth","")}_no_rssi2dist_free.png'
    plt.tight_layout()
    ...
    plt.ylim(0, 1)
    plt.legend(fontsize=SLIDE_LEGEND_FONT_SIZE)
    error_cdf_filename = f'new_results/error_cdf_loaded_model_{load_path.split("/")[-1].replace(".pth","")}_no_rssi2dist_free.png'
    plt.tight_layout()
    ...
    errors_filename = f'new_results/gcn_errors_{load_path.split("/")[-1].replace(".pth","")}_no_rssi2dist_free.npy'
    ...
    eval_dump_filename = f'new_results/eval_dump_{load_path.split("/")[-1].replace(".pth","")}_no_rssi2dist_free.npz'
    np.savez_compressed(
        eval_dump_filename,
