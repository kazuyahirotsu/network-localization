...
offset = Parameter(torch.tensor(0.0, requires_grad=True, device=device))
...
optimizer_gnn = torch.optim.Adam(main_gnn.parameters(), lr=1e-4, weight_decay=1e-5)
optimizer_phys_params = torch.optim.Adam([Pt, path_loss_exponent, offset], lr=1e-5) # Smaller LR for physical params
...
def estimate_distance_from_rssi(measured_rssi, Pt_param, ple_param, offset_param, epsilon=1e-6):
    ...
    exponent_val = torch.clamp(exponent_val, -2.0, 4.0) # Clamping log10(distance) roughly between 0.01m and 100km
...
for epoch in range(num_epochs):
    ...
    for data in train_loader:
        ...
        optimizer_phys_params.zero_grad()
...
#######################################
# -> MODIFICATION: Update save_path to reflect constrained offset
save_path = 'trained_localization_model_64beacons_1000instances_fixed_constrained_offset_and_optimizer.pth'
torch.save({
    'main_gnn_state_dict': main_gnn.state_dict(),
    'Pt': Pt,
    'path_loss_exponent': path_loss_exponent,
    'offset': offset,
    'optimizer_gnn_state_dict': optimizer_gnn.state_dict(),
    'optimizer_phys_params_state_dict': optimizer_phys_params.state_dict(),
